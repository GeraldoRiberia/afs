...
def process_audio(stereo_audio, sample_rate, mic_dist_meters=0.2):
    ...
    inputs = feature_extractor(
        mono_audio, sampling_rate=sample_rate, return_tensors="pt")
...
for i, device in enumerate(devices):
    print(f"Index {i}: {device['name']} (Inputs: {
          device['max_input_channels']}, Outputs: {device['max_output_channels']})")
...
if scarlett_index is None:
    ...
    if len(input_devices) > 0:
        ...
        print(f"-> Using first available input device: {
              input_devices[0][1]['name']} (Index {scarlett_index})")
    else:
